...
video = cv2.VideoCapture("C:/Users/naina/Downloads/Pyhton programs of PRO! classes/Pro107bb3.mp4")
...
def goal_track(img, bbox):
    ...
    cv2.circle(img(c1, c2), 2, blue, 6)
    
    cv2.circle(img(int(p1), int(p2), 2, red, 6))
    ...
    if(distance <= 20):
        cv2.putText(img, "Goal!!"  ,(300,100), cv2.FONT_HERSHEY_SIMPLEX, 4, yellow, 1)
    
    xs.append(c1)
    ys.appned(c2)
            
    for i in range(len(xs)-1):
        cv2.circle(img, xs[i], ys[i], 2, green, 8)
